            ...
            if ($.isEmptyObject(bhamashahNumber.trim())) {
                ...
                $.ajax({
                    ...
                    if (typeof (data.errorcode) === 'undefined') {
                        ...
                        alert('Bhamashah service is not working, co-ordinate with support team for further information!');
                    }
                }).error(function (ex) {
                    console.log(ex);
                });
...
function LoadClaimRequest(cntrl) {
    ...
    if (!$.isEmptyObject(claimTypeId)) {
        $.ajax({
            type: 'GET',
            url: rootURl + "ClaimRequest/LoadClaimRequest",
...
function ddlRequestFor_OnChange(cntrl) { 
    if (cntrl.value == '') {
        $('#divOnBehalfOfOption').addClass('hide');
        $('#txtSSOID').val('');
    }
    else {
        $('#divOnBehalfOfOption').removeClass('hide');
    }
}
