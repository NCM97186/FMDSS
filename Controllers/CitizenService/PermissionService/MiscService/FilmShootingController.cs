...
namespace FMDSS.Controllers.CitizenService.PermissionService.MiscService
{
    ...
    public class FilmShootingController : BaseController
    {
        ...
        public JsonResult getDistrict(string Place, string disttype)
        {
            string actionName = this.ControllerContext.RouteData.Values["action"].ToString();
            string controllerName = this.ControllerContext.RouteData.Values["controller"].ToString();
            Int64 UserID = Convert.ToInt64(Session["UserID"].ToString());
            ...
            try
            {
               // disttype = "isFilmShootong";
               // dtd = cs.District(disttype);
                dtd = cs.AutoSuggestDistrict(Place, disttype); 
        ...
        public ActionResult SubmitFilmshootingPermission(FormCollection fm, HttpPostedFileBase IDProofUrl, string Command, HttpPostedFileBase txtUploadtranscript)
        {
            ...
            try
            {
                ...
                {
                    fs.PlaceID = Convert.ToInt64(fm["hdPlaceId"].ToString());
                    fs.PlaceName = fm["Place"].ToString();
                }
                ...
                if (Command == "Payment")
                {
                    ...
                    if (Session["IsKioskUser"] != null && Convert.ToBoolean(Session["IsKioskUser"]) && Session["KioskCtznName"] != null)
                    {
                        ...
                        _obj.RequestedId = Convert.ToString(Session["RequestId"]);
                        DataTable dtKiosk = _obj.FetchKisokValue(_obj);

                        //********************Code modified by rajkumar on 21-12-2016 for kiosk payment******************
                        ...
                        {
                            _obj.RequestedId = Convert.ToString(Session["RequestId"]);
                            //_obj.KioskCharges = Convert.ToDecimal(dtKiosk.Rows[0]["KisokCharges"]);
                            //_obj.CessPercentage = Convert.ToDecimal(dtKiosk.Rows[0]["CessAmount"]);
                            //_obj.ServiceTax = Convert.ToDecimal(dtKiosk.Rows[0]["ServiceTax"]);
                            //_obj.OtherCharges = Convert.ToDecimal(dtKiosk.Rows[0]["OtherCharges"]);
                            //_obj.PDFDocCharges = Convert.ToDecimal(dtKiosk.Rows[0]["PDFDocumentPrice"]);
                            //_obj.KMLCharges = Convert.ToDecimal(dtKiosk.Rows[0]["KMLDocumentPrice"]);
                            //_obj.SHPCharges = Convert.ToDecimal(dtKiosk.Rows[0]["SHPDocumentPrice"]);
        ...
        public ActionResult getCampArea(Int64 PlaceID)
        {
            ...
            try
            {
                cs.PlaceID = PlaceID;
                dtd = cs.GetCampArea();
                foreach (System.Data.DataRow dr in dtd.Tables[0].Rows)
                {
                    CampArea.Add(new SelectListItem { Text = @dr["Name"].ToString(), Value = @dr["CampFeesID"].ToString() });
                }
                if (dtd.Tables[1].Rows.Count > 0)
                {
                    if (Convert.ToString(dtd.Tables[1].Rows[0]["DivCode"]) != "")
                    {
                        Session["EmitraDivCode"] = dtd.Tables[1].Rows[0]["DivCode"].ToString();
                    }
                }
            }
            catch (Exception ex)
        ...
        public ActionResult SubmitOrganizingCamp(FormCollection fm, HttpPostedFileBase FileNameOfIdProof, string Command)
        {
            ...
            try
            {
                ...
                //}
                if (fm["ddl_Place"].ToString() == "")
                {
                    oc.Location = "";
                }
                else
                {
                    oc.Location = fm["hdPlaceId"].ToString();
                    oc.LocationName = fm["ddl_Place"].ToString();
                }
