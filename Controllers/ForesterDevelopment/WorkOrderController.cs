...
//surve
        ...
        public ActionResult ProgressEntry(string WPID)
        {
           string ID = Encryption.decrypt(WPID);
            ...
            if (!string.IsNullOrEmpty(ID))
            {
                ...
                dt = _obj.SelectSubActivityByActivityID(Convert.ToString(dtf.Rows[0]["Activity"]), Convert.ToString(dtf.Rows[0]["WorkOrderID"]));
                ViewBag.fname = dt;
                foreach (System.Data.DataRow dr in ViewBag.fname.Rows)
                {
                    ddlsubActivity.Add(new SelectListItem { Text = @dr["Sub_Activity_Name"].ToString(), Value = @dr["ID"].ToString() });
                } if (Convert.ToInt32(dtf.Rows[0]["AssetCategoryID"]) > 0)
                {
                    dt = _obj.SelectAssetbyCategoryType(Convert.ToString(dtf.Rows[0]["AssetCategoryID"]));
                    ViewBag.fname = dt;
                    foreach (System.Data.DataRow dr in ViewBag.fname.Rows)
                    {
                        ddlAsset.Add(new SelectListItem { Text = @dr["ASSETNAME"].ToString(), Value = @dr["ASSETID"].ToString() });
                    }
                }
                else if (Convert.ToInt32(dtf.Rows[0]["ProduceTypeID"]) > 0)
                {
                    dt = _obj.SelectProductByProduceType(Convert.ToString(dtf.Rows[0]["ProduceTypeID"]));
                    ViewBag.fname = dt;
                    foreach (System.Data.DataRow dr in ViewBag.fname.Rows)
                    {
                        ddlProduct.Add(new SelectListItem { Text = @dr["ProductName"].ToString(), Value = @dr["ID"].ToString() });
                    }
                }
                _obj = genMP.model(dtf);
                _obj.ID = Convert.ToInt32(ID);
            }
            ViewBag.ddlDivision1 = Division;
            ViewBag.ddlDistrict1 = District;
            ViewBag.ddlBlockName1 = BlockName;
        ...
        public ActionResult ProgressEntry(HttpPostedFileBase CurrentStatusUrl, Workorder objWo, FormCollection frm)
        {
            ...
            if (!string.IsNullOrEmpty(frm["MilestoneSelected"]))
            {
                ...
                objWo.MileStoneID = Convert.ToInt64(frm["hdMilestoneId"]);
            }
            else
            {
        ...
        public ActionResult Create(Workorder objWO, FormCollection frm)
        {
            try
            {
               ...
               //dtwomp.Columns.Add("MicroPlanID", typeof(String));
                dtwomp.Columns.Add("SchemeID", typeof(String));
                dtwomp.Columns.Add("ModelIDs", typeof(String));
                dtwomp.Columns.Add("ActivityIDs", typeof(String));
                dtwomp.Columns.Add("SubActivityId", typeof(String));
                dtwomp.Columns.Add("Quantity", typeof(String));
                ...
                foreach (WorkOrderMicroplan objwomp in womp)
                {
                   ...
                   //dr["MicroPlanID"] = "0";// objwomp.MicroPlanID;
                    dr["SchemeID"] = objwomp.SchemeID;
                    dr["ModelIDs"] = objwomp.ModelIDs;
                    dr["ActivityIDs"] = objwomp.ActivityIDs;
                    dr["SubActivityId"] = objwomp.SubActivityId;
                    dr["Quantity"] = objwomp.Quantity;
                    dtwomp.Rows.Add(dr);
                    dtwomp.AcceptChanges();
                }
        ...
        public JsonResult AddWorkOrderMicroplan(WorkOrderMicroplan WOMP)
        {
            ...
            try
            {
                ...
                if (!lstWOMP.Any(a => a.ActivityIDs == WOMP.ActivityIDs && a.ModelIDs == WOMP.ModelIDs  && a.SchemeID == WOMP.SchemeID && a.SubActivityId==WOMP.SubActivityId))
                {
                    WOMP.rowID = Guid.NewGuid().ToString();
                    if (lstWOMP.Any(a => a.SchemeID == WOMP.SchemeID))
                        WOMP.projectArea = 0;
                    else
                    {
                        WOMP.projectArea = Convert.ToDouble(_obj.Select_ProjectArea(WOMP.SchemeID));// new Random().Next(1, 25);
                    }
                    //if (lstWOMP.Any(a => a.ActivityIDs == WOMP.ActivityIDs && a.ModelIDs == WOMP.ModelIDs))
                    //    WOMP.ActivityCost = 0;
                    //else
                    //{
                        DataTable dtBSRwithUnit = _obj.Select_Activity_BSR(WOMP.ActivityIDs, WOMP.SubActivityId,WOMP.Division);
                    
                        WOMP.ActivityCost = Convert.ToDouble(dtBSRwithUnit.Rows[0]["TotalAmount"]);
                            
                        WOMP.UnitName = dtBSRwithUnit.Columns.Contains("Sub_Activity_Unit")? Convert.ToString(dtBSRwithUnit.Rows[0]["Sub_Activity_Unit"]):"NA";
                        lstWOMP.Add(WOMP);
       ...
       public JsonResult  Select_Activity_BSRAmount(string ActivityID)
        {
            ...
            try
            {
                stramt = _obj.Select_Activity_BSRAmount(ActivityID).Rows[0]["TotalBSR"].ToString();
       ...
       public JsonResult Select_SubActivity_CircleBSRAmount(string SubActivityID, string Range_Code)
        {
            string stramt = "";
            try
            {
                stramt = _obj.Select_SubActivity_CircleBSRAmount(SubActivityID, Range_Code).Rows[0]["TotalAmount"].ToString();
            }
            catch (Exception ex) { Console.Write("Error " + ex); return null; }
            finally { }
            return Json(stramt, JsonRequestBehavior.AllowGet);
        }
       public JsonResult Select_Activity_CircleBSRAmount(string ActivityID, string Range_Code)
        {
            string stramt = "";
            ...
            try
            {
                stramt = _obj.Select_Activity_CircleBSRAmount(ActivityID, Range_Code).Rows[0]["TotalBSR"].ToString();
        ...
        [HttpPost]
        public JsonResult SelectWorkOrderByVillageCode(string Vill_Code)
        {
            try
            {
                DataTable dt = _obj.SelectWorkOrderByVillCode(Vill_Code);
                return dtToViewBagJSON(dt, "WorkOrder_Name", "ID");
            }
            catch (Exception ex) { Console.Write("Error " + ex); return null; }
        }
        ...
        {
           try {  DataTable dt = _obj.SelectProjectByMicroplan(MicroplanID);
        ...
        [HttpPost]
        public JsonResult SelectBudgetHeadByScheme()
        {
            try
            {
                string SchemeIDs = "0";
                ...
                if (Session["DTWOMP"] != null)
                {
                    WOMP = (List<WorkOrderMicroplan>)Session["DTWOMP"];
                    foreach (WorkOrderMicroplan ww in WOMP)
                        SchemeIDs += "," + ww.SchemeID.ToString();
                }
                DataTable dt = _obj.SelectBudgetHeadByScheme(SchemeIDs);
                return dtToViewBagJSON(dt, "BudgetHead", "ID");
            }
            catch (Exception ex) { Console.Write("Error " + ex); return null; }
        }
        [HttpPost]
        public JsonResult SelectModelBySchemeID(string SchemeID)
        {try {
            DataTable dt = _obj.SelectModelBySchemeID(SchemeID);
        ...
        public JsonResult GetUnitByProduct(string ProduceTypeID)
        {
            try
            {
                DataTable dt = _obj.SelectUnitByProduceType(ProduceTypeID);
                return dtToViewBagJSON(dt, "UnitName", "ID");
            }
            catch (Exception ex) { Console.Write("Error " + ex); return null; }
        }
        ...
        [HttpPost]
        public JsonResult SelectActivityByModelID(string ModelID, string DIV_Code)
        {try {
            DataTable dt = _obj.SelectActivityByModelID(ModelID, DIV_Code);
            return dtToViewBagJSON(dt, "Activity_Name", "ID");
        }
        catch (Exception ex) { Console.Write("Error " + ex); return null; }
        }
        ...
        [HttpPost]
        public JsonResult SelectSubActivityByActivityID(string ActivityID, string WorkOrder)
        {
            try
            {
                DataTable dt = _obj.SelectSubActivityByActivityID(ActivityID, WorkOrder);
        ...
        [HttpPost]
        public JsonResult SelectSubActivityByActivityIDWorkOrder(string ActivityID)
        {
            try
            {
                DataTable dt = _obj.SelectSubActivityByActivityIDWorkOrder(ActivityID);
                return dtToViewBagJSON(dt, "Sub_Activity_Name", "ID");
            }
            catch (Exception ex) { Console.Write("Error " + ex); return null; }
        }
