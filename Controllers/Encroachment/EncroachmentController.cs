	...
	public class EncroachmentController : Controller
	{
		...
		public ActionResult Registration(string docId="")
		{
			try
			{
				if(!string.IsNullOrEmpty(docId))
				{
					...
					string path1 = Server.MapPath("~/EnchroachmentGISData/" + docids[0]);
					string path2 = Server.MapPath("~/EnchroachmentGISData/" + docids[1]);
		...
		public ActionResult Submit(EncroachmentView objEncroch, HttpPostedFileBase Files)
		{
			try
			{
				...
				if (!string.IsNullOrEmpty(objEncroch.DOE.ToString()))
				{
					FromDate = Convert.ToDateTime(objEncroch.DOE);
				}
				objEncroch.DOE = Convert.ToDateTime(FromDate);
			...
			{
				Console.WriteLine("Message : " + ex.Message + " Inner Exception :" + ex.InnerException.Message.ToString());
				throw;
			}
		...
		public ActionResult getGISData_Old(FormCollection form)
		{
		...
		public ActionResult getGISData(FormCollection form)
		{
			...
			try
			{
				if (Convert.ToString(form["successFlag"]).ToLower() == "true")
				{
					...
					string path1 = Server.MapPath("~/EnchroachmentGISData/" + filepath1);
					string path2 = Server.MapPath("~/EnchroachmentGISData/" + filepath2);
		...
		public ActionResult GetEncroachmentDetails(string EnchId)
		{
			...
			EncroachmentDetailView NewModel = new EncroachmentDetailView();
			try
			{
				...
				if (model != null)
				{
					NewModel.DIV_NAME = model.FirstOrDefault().DIV_NAME;
					NewModel.RANGE_NAME = model.FirstOrDefault().RANGE_NAME;
					NewModel.LRACTNO = model.FirstOrDefault().LRACTNO;
					NewModel.Area = model.FirstOrDefault().Area;
					NewModel.DOE = model.FirstOrDefault().DOE;
					NewModel.Description = model.FirstOrDefault().Description;
					NewModel.IsKnown = model.FirstOrDefault().IsKnown;
					NewModel.GISFilePath = model.FirstOrDefault().GISFilePath;
					NewModel.CreatedBy = model.FirstOrDefault().CreatedBy;
					var KMLDetailList = model.Distinct().Select(x => new { DIV_NAME = x.DIV_NAME, DIST_NAME = x.DIST_NAME, BLK_NAME = x.BLK_NAME, GPSLat = x.GPSLat, GPSLong = x.GPSLong, AreaShapeinHactare = x.AreaShapeinHactare, Area = x.Area, Vill_Name = x.Vill_Name });
					//NewModel.KMLDetails = (List<KMLDetails>)KMLDetail;
					NewModel.KMLDetails = new List<KMLDetails>();
					foreach (var Item in KMLDetailList.Distinct())
					{
						NewModel.KMLDetails.Add(new KMLDetails { DIV_NAME = Item.DIV_NAME, DIST_NAME = Item.DIST_NAME, BLK_NAME = Item.BLK_NAME, GPSLat = Item.GPSLat, GPSLong = Item.GPSLong, AreaShapeinHactare = Item.AreaShapeinHactare, Area = Item.Area, Vill_Name = Item.Vill_Name });
					}
					var EncroacherDetailList = model.Distinct().Select(x => new { Encroacher_Name = x.Encroacher_Name, Encroacher_FatherName = x.Encroacher_FatherName, Encroacher_Address = x.Encroacher_Address, AadharOrBhamasha = x.AadharOrBhamasha });
					NewModel.EncroacherDetails = new List<EncroacherDetails>();
					foreach (var Item in EncroacherDetailList.Distinct())
					{
						NewModel.EncroacherDetails.Add(new EncroacherDetails { Encroacher_Name = Item.Encroacher_Name, Encroacher_FatherName = Item.Encroacher_FatherName, Encroacher_Address = Item.Encroacher_Address, AadharOrBhamasha = Item.AadharOrBhamasha });
					}
				}
			...
			return PartialView("_GetEnchroachmentDetails", NewModel);
		}
		#endregion
